...
## prompt injection / tool output 混入疑い時の緊急対応（{ISSUE-ID}）
...
### claude-peers 経路のゲート運用（{ISSUE-ID}）
...
## セキュリティ修正系 PR の Codex 二次レビュー（{ISSUE-ID} / {ISSUE-ID}）
...
> 自動検知はあくまで保険。**`[codex-review]` の明示付与が運用の基本**（{ISSUE-ID}）。
