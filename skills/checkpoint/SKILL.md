...
3. 未指定 → ブランチ名から推論（判別関数を使う。旧 `grep -oE '^(feat|fix)/([0-9]+)'` は `feat/{ISSUE-ID}-...` にマッチしないバグがあったため lib 関数に統一）:
   ```bash
   source .claude/skills/checkpoint/lib/resolve-issue-ref.sh
...
> **実機検証**（{ISSUE-ID} / 2026-06-24）: `get_issue(id="{ISSUE-ID}", includeRelations=true)` のレスポンス `attachments[0].url` に `https://github.com/<owner>/<repo>/issues/<N>` 形式の URL が格納され、正規表現で GitHub Issue 番号 `<N>` を抽出できることを確認済み（`attachments[].url` が主経路）。
