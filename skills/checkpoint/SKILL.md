...
model: sonnet
name: checkpoint
description: 作業を中断（pause）して進捗を Issue に保存し、別セッションから復元（resume）するチェックポイント。Issue は GitHub 番号と Linear IDの両方で指定可
user-invocable: true
allowed-tools:
  - Read
  - Glob
...
> **実機検証**（{ISSUE-ID} / 2026-06-24）: `get_issue(id="{ISSUE-ID}", includeRelations=true)` のレスポンス `attachments[0].url` に `{INTERNAL-LINK} が格納され、正規表現で GitHub Issue 番号 `988` を抽出できることを確認済み（`attachments[].url` が主経路）。

> 記録先は常に GitHub Issue（メンテナ確定方針）。Linear 番号は「入力キー」として受理し、メモ自体は解決後の GitHub Issue にコメントする。Linear からは GitHub↔Linear 同期リンクで辿れる。
...
- Close 済み Issue の re-open は行わない（メンテナ判断）