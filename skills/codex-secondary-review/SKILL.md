...
- **無音失敗の可観測性**: 可用性判定（Step 3.5）が pass しても runtime（`task`）が exit 1・出力空で失敗しうる。その際は Step 4.5 で runtime 診断をローカルに出す（PR には投稿しない = 上記「静かに失敗」を維持したまま切り分け材料だけ残す）
...
- **この判定は「起動できるか」の事前チェック**であり、「起動して実際に動くか」までは保証しない。判定 pass 後に runtime が無音失敗（exit 1・出力空）したときの事後の切り分けは **Step 4.5 の runtime 診断**が担う（{ISSUE-ID}。可用性判定 = 事前 / 診断 = 事後 の二層）
...
### Step 4.5: 空応答時の runtime 診断（可観測性・{ISSUE-ID}）

Step 4 の Agent 返り値が**空 / レビュー本文として成立しない**場合（`CODEX_AVAILABLE=1` で spawn 自体は通ったのに何も返らない）、runtime レイヤで無音失敗している。`codex:codex-rescue` は仕様上 **stdout しか返さず**、companion runtime のエラーは **stderr に出て破棄される**ため、そのままでは失敗理由が一切分からない（本 Issue の主眼）。

この場合は runtime 診断を**ローカルに出力**して切り分け材料を残す（PR には投稿しない = 「静かに失敗」を維持）:

```bash
CSR_DIAG="${CLAUDE_PROJECT_DIR:-.}/.claude/skills/codex-secondary-review/lib/codex-diagnostics.sh"
if [ -f "$CSR_DIAG" ] && . "$CSR_DIAG" 2>/dev/null; then
  echo "codex-secondary-review runtime diagnostics:"
...
| キー | 見るポイント |
|------|-------------|
| `codex_bin` | 解決された codex の**実体パス**。cmux shim（`/var/folders/.../cmux-cli-shims/<uuid>/codex`）等、想定と違う codex を掴んでいないか（本 Issue foothold 2） |
| `version` / `version_exit` | `codex --version` の liveness。exit 非ゼロ or 空なら**バイナリ自体が不調**（PATH 解決先が壊れている） |
| `version_stderr` | 非ゼロ時の stderr 先頭行。shim / runtime 起動失敗のヒント |
| `auth_json` | `~/.codex/auth.json` の**実在のみ**（中身は読まない）。`missing` なら未認証が濃厚 → `!codex login` |
| `plugin_cache` | plugin cache の実在。`missing` なら plugin 未インストール |

- 診断は **liveness まで**（`codex --version`・非ネットワーク・非課金）。`task` を実際に走らせる完全再現（`codex exec` 等・API 課金・ネットワーク往復）は、診断で当たりが付かないときの**手動の深掘り手順**として温存する（自動では踏まない）
- 診断を出しても **`/auto-merge` はブロックしない**（二次レビューは参考情報）。本ステップの目的は「なぜ落ちたか」をログに残すこと
- `version` の値が別経路の報告（例: companion `setup --json` の `codex.detail`）と食い違う場合、PATH が別 codex を解決している強いシグナル（実測: skill 側 `0.144.5` × companion 側 `0.135.0`）
...
| 可用性判定 pass 後に runtime が無音失敗（exit 1・出力空） | **Step 4.5 で runtime 診断をローカル出力**（`codex_bin` 実体パス / version liveness / auth 実在 / cache）。PR 非投稿・auto-merge 非ブロックのまま切り分け材料を残す |
| Codex 出力が空 / 異常 | Step 4.5 の診断を出してから exit 0、コメントは投稿しない |
...
- 手動: メンテナや Claude 自身が `/codex-secondary-review <PR#>` で任意のタイミングで起動可能