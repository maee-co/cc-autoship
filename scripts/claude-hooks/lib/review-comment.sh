...
rc_has_review_heading() {
  ...
  printf '%s' "$text" | grep -qE '<!--[[:space:]]*review-verdict:[[:space:]]*(pass|needs-review|fail)[[:space:]]*-->' && return 0
  # --fix のマーカーもここで通す（{ISSUE-ID} Phase 3）。本関数は
  # post-tool-use-auto-merge-after-review.sh の **前段ゲート**で、一致しなければ hook は即 exit 0 し、
  # 後段の rc_is_fix_result_heading 分岐（再 /review リマインド）に到達しない。
  # 日本語の `## レビュー指摘修正結果` は RC_REVIEW_HEADING_PATTERN に含まれるため従来は通過できたが、
  # 本文がセッション言語で書かれるようになると英語 --fix は見出しで一致せず、リマインドが
  # 無言で消える。ここを通しても --fix と通常レビューの区別は後段の専用判定が行う。
  printf '%s' "$text" | grep -qE '<!--[[:space:]]*review-fix-result[[:space:]]*-->'
}
...
# 戻り値: 0 = 含む / 1 = 含まない
#
# 言語不変マーカー <!-- review-fix-result --> を優先し、日本語見出しは後方互換で残す
# （{ISSUE-ID} Phase 3）。レビュー本文がセッション言語で書かれるようになったため、英語セッションの
# --fix コメントは日本語見出しを持たない。見出し文字列だけに依存すると「--fix なのに通常レビュー
# 扱い」に無言で倒れ、auto-merge を誤って促す（#N と同型）。通常レビューの
# <!-- review-verdict: ... --> と対になる別マーカーで、両者は排他に判定される。
rc_is_fix_result_heading() {
  local text="${1:-}"
  printf '%s' "$text" | grep -qE '<!--[[:space:]]*review-fix-result[[:space:]]*-->' && return 0
...
rc_resolve_detection_text() {
  ...
  local body_path=""
  body_path=$(rc_extract_body_file_path "$cmd") || body_path=""
  if [ -n "$body_path" ]; then
    local content
    if content=$(rc_read_body_file "$body_path" "$cwd"); then
      printf '%s' "$content"
      return 0
    fi
