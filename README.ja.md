...
- **ゲート付き自律マージ** — サイズ / スコープ / レビュー / CI（UI 変更は e2e）を満たした PR だけ
- **公開コンテンツの保護** — 公開指定したパスは自動マージしない
- **判定は決定的** — ゲートは LLM プロンプトでなく、テスト済みの bash 純関数
...
> スコープ・危険ファイル判定は `apps/` + `packages/`（モノレポ的）構成を前提にしています。単一パッケージのリポジトリでは単に発火しません。
...
- **モノレポ前提の判定。** スコープ・危険ファイル判定は `apps/` + `packages/` 構成を前提にしています。単一パッケージのリポジトリでは発火しません（fail-open） — 安全側ですが、その分スコープ保護は弱まります。
